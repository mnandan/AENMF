...
double GetFact::getW( ) {
	...
	for (UINT ind = 0; ind < N; ind++) {
		...
		if(ind < R) {
			for (UINT fInd = 0; fInd < xFnum; fInd++) {
				GM[ind][F[fInd].fNum] -= REG_PARAM2*F[fInd].fVal;
				for (UINT ind2 = 0; ind2 < R; ind2++) {
					GM[ind2][F[fInd].fNum] -= H[ind][ind2]*F[fInd].fVal;
				}
			}
			for (UINT fInd = 0; fInd < D; fInd++)
				GM[ind][fInd] += W[ind][fInd]*REG_PARAM2;
		} else {
			for (UINT fInd = 0; fInd < xFnum; fInd++)
				for (UINT ind2 = 0; ind2 < R; ind2++) {
					GM[ind2][F[fInd].fNum] -=H[ind][ind2]*F[fInd].fVal;
	...
	while (iter++ < 1000) {
		...
		for (UINT s = 0; s < R; s++) {
			...
			if(rpCache[vI][vI] != 0) {
				for (UINT fInd = 0; fInd < D; fInd++) {
					double denom = REG_PARAM2 + REG_PARAM + rpCache[vI][vI];
					gPod = GM[vI][fInd]/denom;
					lambdaNew = std::max(W[vI][fInd] - gPod, LAMBDA_MIN2);
					delta = lambdaNew - W[vI][fInd];
					if(delta != 0.0) {
						dat.putWval(vI,fInd,lambdaNew);		//W[vI][fInd] = lambdaNew;
						GM[vI][fInd] += delta*REG_PARAM2;
					}
					lambda[fInd] = delta;
				}
	...
	while (iter++ < 1000) {
		...
		for (UINT s = 0; s < activeSize; s++) {
			...
			if (fabs(PG) > TAU) {
				...
				double lambda2 = std::max(H[hInd][vI] - gVal, LAMBDA_MIN2);
