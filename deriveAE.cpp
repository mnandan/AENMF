...
bool DeriveAE::select_working_set(UINT &out_i, UINT &out_j) {
	double Gmax = -INF;
	double Gmax2 = -INF;
	int Gmax_idx = -1;
	int Gmin_idx = -1;
...
double DeriveAE::getRepErr(double xNorm, UINT hInd) {
	double lInit = (double) 1.0 / rpSize; //should sum to 1
	for (UINT i = 0; i < rpSize; i++) {
		G[i] = -H[hInd][i];
		lambda[i] = lInit;
		index[i] = i;
